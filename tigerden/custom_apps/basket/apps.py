...
class BasketConfig(apps.BasketConfig):
    ...
    def ready(self):
        ...
        self.cancel_request_view = get_class(
            'basket.views', 'RequestCancelView')
    ...
    def get_urls(self):
        urls = [
            ...
            url(r'^saved/$', login_required(self.saved_view.as_view()),
                name='saved'),
            url(r'^minireqscart/$', self.mini_reqs_view.as_view(),
                name='mini-reqs-cart'),
            url(r'^reqscart/$', self.cart_reqs_view.as_view(), name='reqs-cart'),
            url(r'^req/(?P<pk>\d+)/remove/$',
                self.cancel_request_view.as_view(), name='req-cancel'),
        ]
