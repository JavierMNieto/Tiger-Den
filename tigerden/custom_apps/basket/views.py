...
class MiniReqsCartView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated and self.request.user.is_supervisor():
            ctx['order_requests'] = self.request.user.get_order_requests().exclude(
                id__in=self.excluded_reqs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST.get("remove_all", False):
            ...
            if cnt == 1:
                ...
                messages.info(request, _(
                    "%s orders have been cancelled") % (cnt))
        ...
        try:
            ...
            messages.error(request, _(
                "No order found with id '%s'") % order_id)
        else:
            order.set_status('Cancelled')
            messages.info(request, _("%s Order '%s' has been cancelled") % (
                order.user_label, order.number))
