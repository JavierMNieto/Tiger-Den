...
class UserDetailView(views.UserDetailView):
    ...
    def make_credit_acct(self, request, user):
        ...
        account = models.Account.objects.create(
            primary_user=user, name=user.username)
        account.save()
        messages.success(self.request, _(
            "Successfully created credit account"))
    ...
    def change_supervisor(self, request, user):
        if user.is_supervisor():
            ...
            messages.info(
                request, "Removed supervisor permissions from " + user.label())
        else:
            supervisor_group.user_set.add(user)
            messages.info(
                request, "Added supervisor permissions to " + user.label())
